...
class MetaController:
    ...
    def __init__(
        ...
        self.notifier = NotificationService(bot_token=bot_token, chat_id=chat_id)
        self.approval_service = ApprovalService()
        self.advisor = LLMAdvisor()

        self._last_global_regime: str | None = None
        self._daily_summary_sent: str | None = None  # date string
    ...
    async def evaluate(self, db: Session, dataframes: dict[str, pd.DataFrame] | None = None):
        ...
        try:
            ...
            if self._last_global_regime and global_regime != self._last_global_regime:
                ...
                await self.notifier.notify_regime_change(self._last_global_regime, global_regime)
            ...
            if perf_snap.consecutive_losses >= 3:
                await self.notifier.notify_consecutive_losses(perf_snap.consecutive_losses)

            if perf_snap.drawdown_intraday >= 1.5:
                await self.notifier.notify_drawdown_breach(perf_snap.drawdown_intraday, 1.5)

            if perf_snap.api_error_count >= 5:
                await self.notifier.notify_api_errors(perf_snap.api_error_count)

            # 3. Evaluate profile switch
            switch = self.profile_manager.evaluate_switch(perf_dict, global_regime)
            if switch:
                await self._handle_switch(db, switch, perf_dict)

            # Check for previously approved requests that can now be applied
            await self._apply_approved_requests(db)
            ...
            await self._maybe_send_daily_summary(perf_dict)
    ...
    async def _handle_switch(self, db: Session, switch: dict, perf_dict: dict):
        ...
        if switch["requires_approval"]:
            ...
            await self.notifier.notify_approval_required(from_p, to_p, reason, req.id)
            logger.info(
            ...
            if applied:
                await self.notifier.notify_profile_switch(from_p, to_p, reason, perf_dict)
        else:
            logger.info("PROFILE: switch %s → %s evaluated but not auto_apply", from_p, to_p)

    async def _apply_approved_requests(self, db: Session):
            ...
            if approved:
                ...
                if applied:
                    ...
                    await self.notifier.notify_profile_switch(
                        ...
                        profile_name, reason, perf_dict,
                    )

    async def _maybe_send_daily_summary(self, perf_dict: dict):
        """Send daily summary once per day."""
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        if self._daily_summary_sent == today:
            return
        # Send at the first cycle of the day
        now = datetime.now(timezone.utc)
        if now.hour == 0 and now.minute < 20:
            await self.notifier.notify_daily_summary(perf_dict)
